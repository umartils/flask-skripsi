...
GOOGLE_API_KEY = "AIzaSyBUdd4UuPYuU5dJq_TcC12gd9bAky4T0AA"

# Path ke vector store
VECTOR_STORE_PATH = "./chroma_db"

# Model Gemini yang digunakan
GEMINI_MODEL = "gemini-2.5-flash"  # atau "gemini-1.5-pro"
...
def evaluate_rag_with_gemini(
    ...
    Args:
        ...
        model: Model Gemini ("gemini-1.5-flash" atau "gemini-1.5-pro")
    ...
    # Set LLM dan Embeddings untuk setiap metric
    ...
    try:
        ...
        print("="*70)
        for metric, score in results.results_dict():
            # Interpretasi skor
            if score >= 0.8:
                status = "✓ Excellent"
            elif score >= 0.6:
            ...
            print(f"{metric:.<40} {score:.4f}  {status}")
        print("="*70 + "\n")
...
def save_results(results, dataset, output_dir="./evaluation_results"):
    ...
    for metric, score in results.items():
        if score >= 0.8:
            ...
            if metric == "faithfulness":
                recommendation = "Improve prompt template and context quality"
            elif metric == "answer_relevancy":
                recommendation = "Enhance retrieval quality and relevance"
            elif metric == "context_recall":
                recommendation = "Increase retrieval k parameter"
            else:
                recommendation = "Filter irrelevant context, tune retriever"
    ...
    interp_path = os.path.join(output_dir, "evaluation_interpretation.csv")
    df_interpretation.to_csv(interp_path, index=False)
...
# Test cases untuk tanaman anggur
TEST_QUESTIONS = [
...
def main():
    ...
    if results and dataset:
        ...
        results_dict = results.to_dict()
        if avg_score >= 0.8:
            print("🎉 Excellent! Your RAG system performs very well.")
            print("   Continue monitoring and maintaining the quality.")
        elif avg_score >= 0.6:
            print("👍 Good performance! Some areas for improvement:")
            for metric, score in results_dict.items():
                if score < 0.7:
                    print(f"   • Consider improving {metric}")
        else:
            print("⚠️  Performance needs improvement. Priority actions:")
            print("   1. Review and improve prompt templates")
            print("   2. Enhance document quality in vector store")
            print("   3. Tune retriever parameters (k, weights)")
            print("   4. Verify embedding model quality")
