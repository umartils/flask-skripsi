...
# ===== INISIASI MODEL DEEPLEARNING =========
yolo_detector = YOLODetector(model_path="model/yolo.pt", output_dir=PROCESSED_FOLDER)
cnn_detector = CNNDetector(model_path="model/best_fusion_model(light)_(2025-11-06_18-17).h5")
...
def chat():
    ...
    try:
        ...
        db_conn.save_message(session_id, 'user', user_message)

        # 2. Ambil riwayat percakapan
        chat_history_from_db = db_conn.get_history(session_id)
        ...
        # Menggunakan .content untuk mendapatkan string dari AIMessage
        # prompt = f"""You are a helpful AI assistant. Use the conversation history to answer the user's question.
        ...
        # # 4. Panggil model
        # ai_response_obj = chatbot_model.invoke(prompt)
        # ai_response_text = ai_response_obj.content # Ambil teks dari respons
        
        ai_response_text = rag_chatbot.generate_response_text(user_message,rag_context)

        # 5. Simpan balasan AI
        db_conn.save_message(session_id, 'ai', ai_response_text)
...
def upload_image():
    ...
    if not session_id or image_file.filename == '':
    ...
    try:
        ...
        # 🔥 Jalankan deteksi YOLO
            ...
            ai_response_text = rag_chatbot.generate_response_img(class_name)
            db_conn.save_message(session_id, 'ai', ai_response_text)
            return jsonify({
...
def new_session():
    """Endpoint untuk membuat session ID baru"""
